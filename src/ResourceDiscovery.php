...
/**
 * Finds {@see Resource} classes under {@code admin.discover} paths using project {@code composer.json} PSR-4 {@code autoload}.
 */
final class ResourceDiscovery
{
    ...
    {
        /** @var mixed $raw */
        $raw = Repository::get('admin.discover', []);
        if ($raw === true) {
            $default = Repository::basePath() . '/app/Admin/Resources';
    ...
    /**
     * @return array<string, string> composer PSR-4 prefix (as stored in JSON) => relative directory under project root
     */
